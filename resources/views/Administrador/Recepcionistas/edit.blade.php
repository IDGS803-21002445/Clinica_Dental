...
@section('content_header')
    <h1 class="mb-0">Editar recepcionista</h1>
@stop
            ...
            <form method="POST" action="{{ route('recepcionistas.update', $recepcionista) }}">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control @error('nombres') is-invalid @enderror" name="nombres"
                        value="{{ old('nombres', $recepcionista->nombres) }}" />
                    @error('nombres')
                ...
                <div class="form-group">
                    ...
                    <input class="form-control @error('apellidos') is-invalid @enderror" name="apellidos"
                        value="{{ old('apellidos', $recepcionista->apellidos) }}" />
                    @error('apellidos')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control @error('usuario_id') is-invalid @enderror" name="usuario_id">
                        ...
                        @foreach ($usuarios as $u)
                            <option value="{{ $u->id }}" @selected((string) old('usuario_id', $recepcionista->usuario_id) === (string) $u->id)>{{ $u->email }}</option>
                        @endforeach
                    </select>
                    @error('usuario_id')
                        <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="d-flex justify-content-end">
                    <a href="{{ route('recepcionistas.index') }}" class="btn btn-secondary mr-2">Volver</a>
